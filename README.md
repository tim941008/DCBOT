...
ADMIN_NOTIFY_USER_ID=可選，錯誤通知接收者的 Discord User ID
```
...
- `config.py` - 環境變數與設定
- `schema.sql` - Supabase 資料表與新功能欄位 migration
...
- `$加追蹤 <課號> [門檻]`
  - 將指定課號加入目前使用者的追蹤清單
- `$我的追蹤`
  - 分頁顯示目前使用者的追蹤清單，並可用按鈕刪除追蹤
- `$刪追蹤 <課號>`
  - 從追蹤清單移除指定課程
- `$設定門檻 <課號> <數字>`
  - 更新通知門檻
- `$設定頻道 <課號> [#頻道]`
  - 將指定課程的通知同步送到指定頻道；省略頻道時使用目前頻道
- `$清除頻道 <課號>`
  - 清除指定課程的頻道通知
- `$自動移除 <課號> <開|關>`
  - 有名額且通知成功後，自動移除該追蹤
...
- `$衝堂`
  - 檢查追蹤課程是否有相同上課時間
- `$每日摘要`
  - 立即私訊目前追蹤清單摘要；Bot 也會每天 08:00（Asia/Taipei）自動私訊摘要
- `$狀態`
  - 檢查 Discord、Supabase、查課 API、監控任務狀態
- `$管理`
  - 管理員查看追蹤統計與監控狀態
...
notify_enabled BOOLEAN -- 是否啟用名額通知；收藏預設為 false
notify_channel_id TEXT -- 可選，頻道通知目標 Discord Channel ID
auto_remove BOOLEAN    -- 通知成功後是否自動移除追蹤
note TEXT              -- 使用者備註
is_wishlist BOOLEAN    -- 是否為想選清單項目
```

若你使用 Supabase SQL 建表，請直接執行 `schema.sql`。它會補齊新欄位，並為 `(user_id, course_no)` 加上唯一限制，避免同一使用者重複追蹤同一門課。

### 新增/變更的指令
- `$加追蹤 <課號> [門檻]`：將課程加入追蹤，第二參數為可選整數門檻（預設 1），例如 `$加追蹤 12345 2`。
- `$設定門檻 <課號> <數字>`：更新已追蹤項目的通知門檻，例如 `$設定門檻 12345 3`。
- `$查課號 <課號>` 的課程卡片提供 `1 / 2 / 3 / 5` 一鍵門檻按鈕，若尚未追蹤會直接加入追蹤。
- `$收藏 <課號> [備註]` 預設不啟用名額通知；之後使用 `$加追蹤` 或一鍵門檻按鈕會轉為追蹤通知。
- `$設定頻道 <課號> [#頻道]` 需要使用者在伺服器內有管理頻道權限。
- `ADMIN_NOTIFY_USER_ID` 可選；設定後，查課 API 或通知連續失敗時會私訊管理員。
